...
def signup():
    ...
    if existing_user:
        ...
        response.set_cookie('username', name, max_age=360000, expires=True, httponly=True, secure=True)
...
def login():
    ...
    if user_record:
        ...
        if pwd_context.verify(password, stored_hashed_password):
            ...
            response.set_cookie('username', username, max_age=360000, expires=True, httponly=True, secure=True)
            return response
